...
local swipe_style(center, theme, fontSizeConfig, fs=null) = {
  '上划样式': {
    '文字样式': {
...
local ButtonUpForegroundStyles(key, o, theme, center, type, fontSizeConfig) =
...
// 下划前景
local ButtonDownForegroundStyles(key, o, theme, center, type, fontSizeConfig) =
...
// 上划提示气泡
local ButtonSwipeUpHintForegroundStyle(key, o, theme, type, fontSizeConfig) =
  if !std.objectHas(o, 'label') then {} else {
    [if type == 'number' then 'number' + key + 'ButtonSwipeUpHintForegroundStyle' else
      key + 'ButtonSwipeUpHintForegroundStyle']:
      display(o) +
      if std.objectHas(o.label, 'text') then
        swipe_style(center['划动气泡文字偏移'], theme, fontSizeConfig)['上划气泡前景样式']
      else
        swipe_style(center['划动气泡sf符号偏移'], theme, fontSizeConfig)['上划气泡sf符号前景样式'],
  };
// 下划提示气泡
local ButtonSwipeDownHintForegroundStyle(key, o, theme, type, fontSizeConfig) =
  if !std.objectHas(o, 'label') then {} else {
    [if type == 'number' then 'number' + key + 'ButtonSwipeDownHintForegroundStyle' else
      key + 'ButtonSwipeDownHintForegroundStyle']:
      display(o) +
      if std.objectHas(o.label, 'text') then
        swipe_style(center['划动气泡文字偏移'], theme, fontSizeConfig)['下划气泡前景样式']
      else
        swipe_style(center['划动气泡sf符号偏移'], theme, fontSizeConfig)['下划气泡sf符号前景样式'],
  };

// 按下气泡
local ButtonHintForegroundStyle(key, o, theme, type, fontSizeConfig) = {
  [key + 'ButtonHintForegroundStyle']:
    // { text: if type == 'cn' then std.asciiUpper(key) else key } +  // 英文使用小写字母
    { text: if Settings.is_letter_capital then std.asciiUpper(key) else key} + // 根据选择的大小写显示气泡的大小写
    swipe_style(center['划动气泡文字偏移'], theme, fontSizeConfig)['按下气泡样式'],
};
...
local finalStyles(type, theme, swipe_up, swipe_down, fontSizeConfig) = {
  ...
  style: std.foldl(  //上划样式
           function(acc, key) acc + ButtonUpForegroundStyles(key, swipe_up[key], theme, centerUp, type, fontSizeConfig),
           std.objectFields(swipe_up),
           {}
         ) +
         std.foldl(  //下划样式
           function(acc, key) acc + ButtonDownForegroundStyles(key, swipe_down[key], theme, centerDown, type, fontSizeConfig),
           std.objectFields(swipe_down),
           {}
         ) +
         std.foldl(  // 上划提示气泡样式
           function(acc, key) acc + ButtonSwipeUpHintForegroundStyle(key, swipe_up[key], theme, type, fontSizeConfig),
           std.objectFields(swipe_up),
           {}
         ) +
         std.foldl(  // 下划提示气泡样式
           function(acc, key) acc + ButtonSwipeDownHintForegroundStyle(key, swipe_down[key], theme, type, fontSizeConfig),
           std.objectFields(swipe_down),
           {}
         ) +
         if type != 'number' then
