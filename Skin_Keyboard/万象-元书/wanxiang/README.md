...
- `swap_9_123_symbol`
  - 控制 9 键底行左下角 `123` 按钮与符号按钮是否交换位置。
  - 默认值：`false`，即保持原始位置。
  - 该配置同时影响两个按钮对应的槽位尺寸，不只是交换显示顺序。
  - 该配置同时作用于竖屏 9 键与横屏 9 键左右下角按钮。

- `swap_numeric_return_symbol`
  - 控制数字键盘左侧返回按钮与右侧切换键盘按钮是否交换位置。
  - 默认值：`false`，即保持原始位置。
  - 该配置同时影响两个按钮对应的槽位尺寸，不只是交换显示顺序。
  - 该配置同时作用于竖屏数字键盘与横屏数字键盘。
...
- `function_button_config`
  ...
  - 包含：
    ...
    - `order`：控制功能行按钮顺序。
  - 其中 `with_functions_row.iPhone` 也会影响横屏 9 键：
    - 开启时，横屏 9 键左右上方显示两半功能按键。
    - 关闭时，横屏 9 键隐藏这两行功能按键，并将下方内容区域自动扩展到完整高度。
  - `order` 可用值：
    - `left`
    - `head`
    - `select`
...
- 横屏 9 键现在使用独立布局，不再复用竖屏布局。
- 整体结构参考横屏 26 键：
  - 左侧区域：上方一半功能按键；下方为 `collection` 与 `verticalCandidates`
  - 中间区域：留白分隔
  - 右侧区域：上方另一半功能按键；下方为完整 9 键输入区
- 左下角按钮与右下角按钮继续共用 `swap_9_123_symbol` 配置。
- 横屏 9 键中的纵向候选区域使用固定名称 `verticalCandidates`，不要随意改名前缀。

## 数字键盘横屏布局

- 横屏数字键盘现在使用独立布局，不再复用竖屏布局。
- 整体结构参考横屏 9 键：
  - 左侧区域：上方一半功能按键；下方拆成两列
    - 第一列：`collection + 返回/切换按钮`
    - 第二列：独立符号面板
  - 中间区域：留白分隔
  - 右侧区域：上方另一半功能按键；下方为完整数字键盘输入区
- 左侧返回按钮与右侧切换键盘按钮共用 `swap_numeric_return_symbol` 配置。
- `function_button_config.with_functions_row.iPhone` 同样会控制横屏数字键盘顶部功能按键显隐，以及下方内容区高度补满。
- 左侧第二列符号面板由 `numeric9Builder.libsonnet` 提供，当前使用 `categorySymbols` 组件类型。
