// 工具栏按钮注册表：统一维护可配置按钮 ID、滑动按钮样式名和滑动按钮 action。
local shared = import 'shared.libsonnet';

{
  // 这里的 action 主要供 horizontalSymbols 滑动按钮使用。
  // 固定按钮仍然依赖 index.libsonnet 里的样式对象自带 action，两者需要保持语义一致。
  getPhoneRegistry(toolbarMenu):: {
    script: {
  ...
  getIpadRegistry(toolbarMenu)::
    // iPad 与 iPhone 保持同一组可配置按钮，只在布局层区分展示方式。
    self.getPhoneRegistry(toolbarMenu),
}
